...
# This is a version of the viterbi algorithm used in a wikipedia example, further modified by stackoverflow users kvorobiev and Ruzihm
...
class NamedEntityRecogniser:
    ...
    def _add_counts(self, sentence, label_sequence):
        ...
        """
        # Counting labels and their occurences
        l_counts = defaultdict(int)
        for s in label_sequence:
            for i in range(0, len(s)):
                l_counts[s[i]] += 1
        self.label_counts = l_counts

        # Counting tokens
        vocab = [w for s in sentence for w in s]
        self.vocab = list(set(vocab))

        # Finding all labels used in text
        L_vocab = [w for s in label_sequence for w in s]
        self.labels = list(set(L_vocab))

        # Counting transitions
        t_counts = defaultdict(int)
        for s in label_sequence:
            for i in range(1, len(s)):
                t_counts[(s[i - 1], s[i])] += 1
        self.transition_counts = t_counts

        # Counting emissions
        e_counts = defaultdict(int)
        for s, t in zip(label_sequence, sentence):
            for v, w in zip(s, t):
                e_counts[(v, w)] += 1
    ...
    def _fill_probs(self, alpha_smoothing=1e-6):
        ...
        # Calculating probabilities the same way as shown in the gruppetime 8 script
        # Other ways i used to create the probabilities are commented out,
        # as the viterbi algorithm does not work, one of the commented out solutions might be more correct
        e_probs = defaultdict(dict)
        for ti in self.labels:
            data = []
            e = defaultdict(dict)
            for w in self.vocab:
                poss = (self.emission_counts[(ti, w)] + alpha_smoothing) / (
                    self.label_counts[ti] + (alpha_smoothing * len(self.vocab))
                )
                # self.emission_probs[(ti, w)] = poss
                # data.append(poss)
                # e_probs[w] = data
                e[w] = poss
            self.emission_probs[ti] = dict(e)
        # self.emission_probs = dict(e_probs)

        # Calculating transition probabilities. Regarding comments, same as emission
        t_probs = defaultdict(dict)
        for w in self.labels:
            data = []
            t = defaultdict(dict)
            for ti in self.labels:
                poss = (self.transition_counts[(ti, w)] + alpha_smoothing) / (
                    self.label_counts[ti] + alpha_smoothing * len(self.vocab)
                )
                ...
                # data.append(poss)
                # self.transition_probs[(ti, w)] = poss
                # data.append(poss)
                # t_probs[ti] = data
            self.transition_probs[w] = dict(t)
            # print(t)
            # print(w, data)
        # self.transition_probs = dict(t_probs)
    ...
    def _viterbi(self, sentence):
        ...
        lattice = [{label: None for label in self.labels} for _ in range(len(sentence))]
        backpointers = [
            {label: None for label in self.labels} for _ in range(len(sentence))
        ]
        for i, token in enumerate(sentence):
            for label in self.labels:

                # print(token, label)
                # print(self.emission_probs[token]["<s>"])
                # print(label, self.transition_probs[("<s>", label)])
                if i == 0:
                    lattice[i][label] = (
                        self.emission_probs[label][token]
                        * self.transition_probs["<s>"][label]
                    )
                else:
                    lattice[i][label] = (
                        self.emission_probs[label][token]
                        * self.transition_probs[label][label]
                    )
                    backpointers[i][label] = np.argmax(lattice[i - 1])
        ...
        for i in range(i, 0, -1):
            ...
            best_path.insert(0, backpointers[i][a])
        
        # Here i got key_error [0], and were not able to find the reason
        """

        # This is a version of the viterbi algorithm used in a wikipedia example, further modified by stackoverflow users kvorobiev and Ruzihm
        # https://stackoverflow.com/questions/35716235/how-to-apply-viterbi-algorithm-in-python
        start_p = lattice[0]
        best_path, best_final_prob = viterbi(
            sentence, self.labels, start_p, self.transition_probs, self.emission_probs
        )
...
def get_BIO_sequence(spans, sentence_length):
    ...
    ['O', 'B-ORG', 'I-ORG', 'O', 'O', 'O']"""
    text_list = [["<s>"]]
    for x, y in zip(spans, sentence_length):
        s = 0
        sentence_list = []
        for z in x:
            for i in range(s, z[0]):
                sentence_list.append("O")
                s = z[0]
            sentence_list.append("B-" + str(z[2]))
            s += 1
            for i in range(z[0] + 1, z[1]):
                sentence_list.append("I-" + str(z[2]))
                s += 1
...
def get_spans(label_sequence):
    ...
    while i < len(label_sequence):
        ...
        if label.startswith("B-"):
            ...
            while end < len(label_sequence) and label_sequence[end].startswith(
                "I-%s" % label
            ):
                end += 1
...
with io.open("norne_train.txt", "r", encoding="utf-8") as text_file:
    text = text_file.read()
# Preprosessing
sentences, spans = preprocess(text)
bio = get_BIO_sequence(spans, sentences)


L = NamedEntityRecogniser()
# print(a[0])
L._add_counts(sentences, bio)
L._fill_probs()
print(
    L._viterbi(
        ["Kjell", "Magne", "Bondevik", "var", "statsminister", "i", "Norge", "."]
    )
)

# Troubleshooting code:
# q = L.transition_probs
# print(q)
# print(L.emission_probs)
